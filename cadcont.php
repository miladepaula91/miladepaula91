...
if(isset($_POST['submit'])){
    ...
    if(empty($nome) || empty($peso)){
        echo "Nome ou peso não podem ser vazios!";
        exit();
    }

    // Use prepared statements para evitar SQL injection
    $result = mysqli_prepare($con, "INSERT INTO disp (nome, peso) VALUES (?, ?)");

    if ($result) {
        mysqli_stmt_bind_param($result, "ss", $nome, $peso); // "ss" significa que estamos passando duas strings sendo nome e peso

        if(mysqli_stmt_execute($result)){//se for incluso dados
            echo "Cadastro realizado!";//exibira essa mensagem
        } else {//se não
            echo "Erro ao cadastrar: " . mysqli_stmt_error($result);//exibira essa
        }

        mysqli_stmt_close($result);// feita consulta em resultados
    } else {//se não existir dados 
        echo "Erro ao preparar a consulta: " . mysqli_error($con);//aparecera essa mmensagem
    }
}
...
<img src="23.jpg" width="550" height="170">
    ...
    <div class="box">
        ...
        <form action="cadcont.php" method="POST">
    ...
    <fieldset>
       ...
       <!-- <legend><b>Cadastro de estabelecimento</b></legend>
        ...
        <legend><b>Cadastro de container</b></legend>
        <br>
        <br>
        <br>
        <br>
